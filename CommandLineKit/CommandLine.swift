...
public class CommandLine {
    ...
    public func parse(strict: Bool = false) throws {
        ...
        for (idx, arg) in argumentsEnumerator {
            ...
            let longFlag = arg.hasPrefix(longOptionPrefix)
            let skipChars = longFlag ? longOptionPrefix.count : shortOptionPrefix.count
            ...
            for option in _options where option.flagMatch(String(flag), long: longFlag) {
            ...
            if !flagMatched && !arg.hasPrefix(longOptionPrefix) {
                ...
                for (i, c) in flagCharactersEnumerator {
                    for option in _options where option.flagMatch(String(c), long: longFlag) {
